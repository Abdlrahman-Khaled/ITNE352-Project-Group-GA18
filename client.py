...
def display_Headlines_Details(data):
    ...
    print(f"Publish Time: {data['publish_time']}")
...
def handle_Headlines_Menu(CSocket):
    while True:
        ...
        if user_Choice=="1":
            ...
            request={"type":"headlines", "option":"category", "value":Categories[index-1]}
        ...
        if request:
            ...
            if response.get("status")== "success" :
                ...
                if headlines:
                    ...
                    if detail !="0":
                        detail_Request={"type":"headline_details","index":detail}
                        detail_Response=send_Request(CSocket,detail_Request)
...
def handle_Sources_Menu(CSocket):
    while True:
        ...
        if user_Choice == "1":
            ...
            request = {"type": "sources", "option": "language", "value": Languages[index - 1]}
        ...
        if request:
            ...
            if response.get("status") == "success":
                ...
                if sources:
                    detail = input("\nenter number for details or (0 to skip)")
                    if detail != "0":
                        detail_Request = {"type": "source_details", "index": detail}
                        detail_Response = send_Request(CSocket, detail_Request)
    ...
    try:
            ...
            while True:
                ...
                if user_Choice=="1":
                    ...
                    quit={"type":"quit"}
                    CSocket.sendall(json.dumps(quit).encode("UTF-8"))
                    print("\nBye")
                    break
                else:
        ...
        print("can't connect to server,The server is running?")
